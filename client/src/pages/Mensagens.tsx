...
import {
  ...
  MoreVertical, Phone, Video, Info, Smile
} from "lucide-react";
...
// ─── Local in-memory store (used until backend persists messages) ─────────────
const _msgs: Message[] = [];
let _msgId = 1;

function getConversationKey(a: number, b: number) {
  return [Math.min(a, b), Math.max(a, b)].join("_");
}

function getMessages(userId: number, otherId: number): Message[] {
  const key = getConversationKey(userId, otherId);
  return _msgs.filter(m => getConversationKey(m.senderId, m.receiverId) === key);
}

function sendMessage(msg: Omit<Message, "id" | "createdAt" | "read">): Message {
...
export default function Mensagens() {
  ...
  const currentUserRole = (user as any)?.role ?? "estagiario";

  // Filter users to show (exclude self, show tutors/admins for estagiarios, show all for tutors/admins)
  const allUsers = usersQuery.data ?? [];
  const contacts = allUsers.filter(u => {
    if ((u as any).id === currentUserId) return false;
    if (search) return (u as any).name?.toLowerCase().includes(search.toLowerCase());
    return true;
  });
  ...
  useEffect(() => {
    ...
    setMessages(getMessages(currentUserId, selectedUserId));
    // Mark as read
    _msgs.forEach(m => {
      if (m.senderId === selectedUserId && m.receiverId === currentUserId) m.read = true;
    });
  ...
  const selectedContact = allUsers.find(u => (u as any).id === selectedUserId) as any;

  // Build conversations list (contacts with last message info)
  const conversations: Conversation[] = contacts.map(c => {
    const cid = (c as any).id;
    const msgs = getMessages(currentUserId, cid);
    const last = msgs[msgs.length - 1];
    const unread = msgs.filter(m => m.senderId === cid && !m.read).length;
    return {
      userId: cid,
      userName: (c as any).name ?? c.openId,
      userRole: (c as any).role ?? "estagiario",
      lastMessage: last ? (last.fileName ? `📎 ${last.fileName}` : last.text) : "",
  ...
  const handleSend = () => {
    if ((!text.trim() && !fileUploading) || !selectedUserId) return;
      ...
      <div className="h-[calc(100vh-4rem)] flex overflow-hidden bg-slate-50 dark:bg-slate-950 -m-4 md:-m-6">
        ...
        <div className={`w-full md:w-80 lg:w-96 flex-shrink-0 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col ${selectedUserId ? "hidden md:flex" : "flex"}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {!usersQuery.isLoading && conversations.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
                <Users size={40} className="text-slate-200 dark:text-slate-700 mb-3" />
                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Sem contactos disponíveis</p>
                <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">Os utilizadores aparecerão aqui</p>
              </div>
            )}
            ...
            {conversations.map(conv => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3.5 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors text-left relative
                  ${selectedUserId === conv.userId ? "bg-red-50 dark:bg-red-950/20 border-r-2 border-red-500" : ""}`}
              >
                <div className="relative flex-shrink-0">
                  <Avatar name={conv.userName} id={conv.userId} />
                  {conv.online && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-sm text-slate-900 dark:text-white truncate">{conv.userName}</span>
                    {conv.lastAt && (
                      <span className="text-[11px] text-slate-400 flex-shrink-0 ml-2">{formatTime(conv.lastAt)}</span>
                    )}
                  </div>
                  <div className="flex items-center justify-between mt-0.5">
                    <span className="text-xs text-slate-400 truncate flex-1">{conv.lastMessage || roleLabel(conv.userRole)}</span>
                    {conv.unread > 0 && (
                      <span className="ml-2 flex-shrink-0 w-5 h-5 bg-red-600 text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                        {conv.unread}
                      </span>
                    )}
                  </div>
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="h-16 px-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  <Avatar name={selectedContact?.name ?? ""} id={selectedContact?.id ?? 0} />
                </div>
                <div>
                  <p className="font-semibold text-sm text-slate-900 dark:text-white">{selectedContact?.name}</p>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-1">
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center py-16">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Nenhuma mensagem ainda</p>
                  <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">Envia uma mensagem para iniciar a conversa</p>
                </div>
              )}
                                  ...
                                  <>
                                    ...
                                    {msg.fileUrl && (
                                      <a
                                        ...
                                        className={`p-1.5 rounded-lg transition-colors flex-shrink-0 ${isMine ? "hover:bg-red-700/50 text-red-100" : "hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-500"}`}
                                        title="Descarregar"
                                      >
                                        <Download size={14} />
                                      </a>
            ...
            <div className="p-3 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex-shrink-0">
              <div className="flex items-end gap-2 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 px-3 py-2.5 focus-within:border-red-400 focus-within:ring-2 focus-within:ring-red-500/20 transition-all">
                ...
                <button
                  onClick={handleSend}
                  disabled={!text.trim() && !fileUploading}
              ...
              <p className="text-[11px] text-slate-400 text-center mt-1.5">
                Enter para enviar · Shift+Enter para nova linha · Clica no <Paperclip size={10} className="inline" /> para anexar ficheiros
              </p>
            </div>
          </div>
        )}
        ...
        {selectedUserId && showInfo && selectedContact && (
          <div className="hidden lg:flex w-72 flex-shrink-0 border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex-col">
            <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              ...
              <button onClick={() => setShowInfo(false)} className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
                <X size={16} />
              </button>
            </div>
            <div className="p-6 flex flex-col items-center text-center border-b border-slate-100 dark:border-slate-800">
              <Avatar name={selectedContact.name} id={selectedContact.id} size="lg" />
              <h3 className="mt-3 font-bold text-slate-900 dark:text-white">{selectedContact.name}</h3>
              <span className={`mt-1.5 text-xs px-2.5 py-1 rounded-full font-medium ${roleBadgeColor(selectedContact.role ?? "")}`}>
                {roleLabel(selectedContact.role ?? "")}
              </span>
              {selectedContact.email && (
                <p className="mt-2 text-xs text-slate-400">{selectedContact.email}</p>
              )}
              {selectedContact.department && (
                <p className="text-xs text-slate-400">{selectedContact.department}</p>
              )}
            </div>
            <div className="p-4">

      ...
      <div className="flex-1 flex overflow-hidden bg-slate-50 dark:bg-slate-950 -m-4 md:-m-6">
        ...
        {!selectedUserId ? (
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            {/* Chat Header */}
            <div className="h-16 px-4 border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm flex items-center justify-between flex-shrink-0 sticky top-0 z-10">
              ...
              {groupedMessages.map(group => (
                <div key={group.date}>
                  ...
                  {group.msgs.map((msg: any, idx: number) => {
                    ...
                    return (
                      <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"} ${isFirst ? "mt-4" : "mt-0.5"}`}>
                        {!isMine && isFirst && (
                        ...
                        <div className={`max-w-[72%] flex flex-col ${isMine ? "items-end" : "items-start"}`}>
                          ...
                          <div className={`rounded-2xl px-3.5 py-2.5 shadow-sm
                            ${isMine
                              ? `bg-red-600 text-white ${isFirst ? "rounded-tr-md" : ""}`
                              : `bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 border border-slate-100 dark:border-slate-700 ${isFirst ? "rounded-tl-md" : ""}`
                            }`}
                          >
                            {/* File attachment */}
                            {msg.fileName && (
